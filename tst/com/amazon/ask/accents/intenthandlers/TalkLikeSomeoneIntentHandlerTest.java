...
public class TalkLikeSomeoneIntentHandlerTest {
        ...
        public void testHandle() {
                ...
                Slot languageSlot = buildSlot(Slots.LANGUAGE_SLOT, languageSlotRawValue, languageSlotResolvedValue,
                                languageSlotId);
                Slot genderSlot = buildSlot(Slots.GENDER_SLOT, genderSlotRawValue, genderSlotResolvedValue,
                                genderSlotId);
        ...
        /*
         * Test that handle does not return APL directives when the device does not
         * support APL interface.
         */
        @Test
        public void testHandle_NoSupportForAPL() {
                // Arrange
                Slot languageSlot = buildSlot(Slots.LANGUAGE_SLOT, languageSlotRawValue, languageSlotResolvedValue,
                                languageSlotId);
                Slot genderSlot = buildSlot(Slots.GENDER_SLOT, genderSlotRawValue, genderSlotResolvedValue,
                ...
                // Remove support for APL.
                RequestEnvelope requestEnvelope = input.getRequestEnvelope();
                SupportedInterfaces supportedInterfaces = SupportedInterfaces.builder().build();
                when(requestEnvelope.getContext().getSystem().getDevice().getSupportedInterfaces())
                                .thenReturn(supportedInterfaces);
        ...
        private Slot buildSlot(String slotName, String slotRawValue, String slotResolvedValue, String slotId) {
                ...
                if (slotResolvedValue != null && slotId != null)
                        resolvedValueWrapper = ValueWrapper.builder()
                                        .withValue(Value.builder().withId(slotId).withName(slotResolvedValue).build())
                                        .build();
                else
                ...
                Resolutions resolutions = Resolutions.builder().withResolutionsPerAuthority(resolutionsPerAuthority)
                                .build();
        ...
        private HandlerInput buildHandlerInput(Map<String, Slot> slots, String intentName) {
                RequestEnvelope requestEnvelope = mock(RequestEnvelope.class, Mockito.RETURNS_DEEP_STUBS);

                AlexaPresentationAplInterface alexaPresentationAPL = AlexaPresentationAplInterface.builder().build();
                SupportedInterfaces supportedInterfaces = SupportedInterfaces.builder()
                                .withAlexaPresentationAPL(alexaPresentationAPL).build();
                when(requestEnvelope.getContext().getSystem().getDevice().getSupportedInterfaces())
                                .thenReturn(supportedInterfaces);
