...
public class CancelIntentHandlerTest {
    ...
    public void testCanHandle_IncorrectIntentName() {
        ...
        IntentRequest intentRequest = mock(IntentRequest.class, Mockito.RETURNS_DEEP_STUBS);
        when(input.getRequestEnvelope().getRequest()).thenReturn(intentRequest);
        when(intentRequest.getIntent().getName()).thenReturn("anIntentThatIsNotCancelIntent");
        ...
        // Assert
        assertFalse("canHandle should return false when any intent other than CancelIntent is passed.", actualValue);
    }
